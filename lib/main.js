...
*/
    ...
    if (new_article.chapters){
        ...
        if (new_article.chapters['published'] > old_article.chapters['published']){
            ...
            old_article.hasupdate__ts = new Date().getTime();
        }
    ...
    var sendData = Request({
        ...
        onComplete: function (response) {
            ...
            if ((response.status == 200) || (response.status == 201)){
                ...
                console.log(JSON.stringify(simpleStorage.storage.ficdict));
            }
        }
    });
    sendData.post();
...
function runSync(){
    ...
    if (prefs['sync_enabled']){
        ...
        if (Date.now() -  minSyncWait < prefs['last_sync']){
            console.log('trying to sync too soon');
            return;
        }
        if (getUser()){
...
function syncWork(data){
    ...
    console.log(backendUrl + 'user/' + user_id + "/work/" + data['ao3id']);
...
function validateAndSaveToken(token, port){
    ...
    var checkToken = Request({
        ...
        contentType: 'application/json',
        onComplete: function (response) {
            console.log('checkToken' + response.status + ' ' + JSON.stringify(response.json));
            // Merge data here
