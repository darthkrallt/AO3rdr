...
*/
    ...
    if (!metadata['ao3id']){
        ...
        console.log('No work changed- No valid ID');
        return null;
    }
    if (mutable_data){
        console.log(Object.keys(mutable_data));
    }
    ...
    if (!(newArticle.ao3id in simpleStorage.storage.ficdict)) {
        ...
        if (!mutable_data){
            console.log('No work changed - only view data, this is not in our DB');
            return null;
        } else {
            ...
            if (arrayCompare(mutable_keys, ['visit'])){
                console.log('No work changed - crawled, ERROR sending visit data');
                return null;
            }
        }
